...
const DashboardView = ({ insights }) => {
  ...
  // Transform salary data for the chart (in Lakhs for display)
  const salaryData = insights.salaryRanges.map((range) => ({
    name: range.role,
    min: range.min / 100000, // Convert to Lakhs
    max: range.max / 100000, // Convert to Lakhs
    median: range.median / 100000, // Convert to Lakhs
  }));

  // Format salary in INR
  const formatSalaryINR = (amount) => {
    ...
    <div className="w-full py-10 md:py-20">
      ...
      <motion.div variants={item}>
        <Card className="col-span-4 border-2 border-muted bg-background/60 hover:border-primary hover:shadow-xl transition-all duration-300 ease-in-out backdrop-blur-sm">
          <CardHeader>
            <CardTitle className="text-2xl font-bold">Salary Ranges by Role (India)</CardTitle>
            <CardDescription>
              Displaying minimum, median, and maximum annual salaries in Lakhs (₹)
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="h-[400px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={salaryData}>
                          ...
                          <div className="bg-background/80 backdrop-blur-sm border rounded-lg p-3 shadow-xl">
                            ...
                            {payload.map((item) => (
                              <p key={item.name} className="text-sm text-muted-foreground">
                                {item.name}: ₹{item.value.toFixed(1)}L
                              </p>
                            ))}
                          </div>
                        );
                  ...
                  />
                  <Bar dataKey="min" fill="rgba(148, 163, 184, 0.7)" name="Min (Lakhs ₹)" />
                  <Bar dataKey="median" fill="rgba(100, 116, 139, 0.8)" name="Median (Lakhs ₹)" />
                  <Bar dataKey="max" fill="rgba(71, 85, 105, 0.9)" name="Max (Lakhs ₹)" />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
